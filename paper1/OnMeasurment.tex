...
Each observable through the frame embeddings, induces a global random variable 
on the joint context, which for convinience will
denote with $\left(\Omega, \mathcal F\right)$. 
The representative of the observable in the joint frame is:
\[
    \widetilde{O}_i := O_i \circ \iota_i^{-1} \Omega \to \mathcal O
\]
...
$\widetilde{O}_i: \Omega \to \mathcal O$, where the only physically admissible events are
those in the pointer algebra, in particular the frame pointer algebra of the frame of
that observable.

Consequently, only those values of $\widetilde{O}_i$ whose events are compatible with
$\mathcal F_i^{\mathrm{ptr}}$ can occur as outcomes. The coarse graining of that algebra
implies proability in the description of the event as per \cref{rem:probability-origin},
and if a measurement is repeated the system remains conditioned on that event for the
interaction. If a new \textbf{incompatible} interaction takes place, coarse-graining
implies a new introduction of distinctions for th previous operator.
