...
\begin{definition}[Pointer Algebra]
    ...
    \]
\end{definition}
\subsection{Symmetry Breaking: Contextuality}
Now we look at what Axiom~1 implies for the joint frame, and its relation to contextuality,
how the relativity principle is violated and what remains physically admissible.
\begin{proposition}[Contextuality violates RIF]
    Suppose a joint frame $\mathcal J_I$ with the full $\sigma$-algebra of the contexts
    automatically satisfies RIF for every event. Then, up to \textbf{automorphisms} of the
    joint frame, the joint frame is non-contextual.
\end{proposition}
\begin{proof}
\end{proof}
\begin{proposition}[An non-contextual joint frame satisfies RIF]
    Suppose there there is a non-contextual choice of embeddings for a joint frame. Then
    on that joint frame, RIF is satisfied for every event $E$ and every context.
\end{proposition}
\begin{proof}
\end{proof}
Together these two imply that, any contextual family of frames \textbf{fails to satisfy 
RIF for every event}. Then Axiom~1 forces us to discard events that are not physically
admissible which leads to the following definition.
\begin{definition}[Joint Pointer Frame]
    The physicaly admissible information frame representing the interaction frame is the
    \textbf{Joint Pointer Frame}, given by:
    \[
        \mathcal J_I^{\text{ptr}} := \left(\Omega_I, \mathcal F_{\text{ptr}}\right)
    \]
\end{definition}
In a sense, the interaction "collapses" to the pointer algebra.
...
\end{proof}
We also can use the same empirical contextuality to show that, any algebra larger than
RIF is nescessarely contextual.
\begin{lemma}[Larger $\sigma$-algebras then the pointer algebra are contextual]
    Let $\mathcal G$ be a sub-$\sigma$-algebra of the full joint frame
    $\mathcal F_{\mathcal J_I}$. Then there exists a empirical family 
    $\left\{\mu_i\right\}_{i \in I}$ for which the joint space with $\mathcal G$ is
    contextual.
\end{lemma}
\begin{proof}
\end{proof}
